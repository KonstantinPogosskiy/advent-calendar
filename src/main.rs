...
fn part_1_day_1() -> Result<(), Box<dyn std::error::Error>> {
    let list = fs::read_to_string("src/day_1.txt")?;
...
fn part_1_day_2() -> Result<(), Box<dyn std::error::Error>> {
    let list = fs::read_to_string("src/day_2.txt")?;
    let array = list.lines()
        ...
        //.len() есть только у коллекциий, а можно посчитать длину элементов самого итератора .count() без создания в памяти коллекции
        .count();
        // .collect::<Vec<_>>().len();
